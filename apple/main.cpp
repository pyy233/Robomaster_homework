...
int main()
{
    ...
    red_sub_green=apple_channels[2]-apple_channels[1];      //红绿通道相减
    normalize(red_sub_green, normal, 0., 255., cv::NORM_MINMAX);        //归一化
    threshold(normal, binary, 31, 255, THRESH_BINARY);                  //二值化
    medianBlur(binary, blured_img1, 37);                                //中值滤波
    Mat dilate_kernel = getStructuringElement(MORPH_RECT,Size(43,43), Point(-1, -1)); //膨胀核
    Mat erode_kernel = getStructuringElement(MORPH_RECT,Size(41,41), Point(-1, -1));  //腐蚀核
    ...
    medianBlur(eroded_img, blured_img2, 103);//中值滤波
